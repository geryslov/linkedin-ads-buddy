  ...
  try {
    ...
    switch (action) {
      case 'get_auth_url': {
        ...
        // Keeps r_liteprofile because `get_profile` calls /v2/me, which requires
        // it — dropping it here would break this dashboard.
        //
        // The standalone MCP product asks for the OIDC set instead
        // (openid profile email …) from its own `mcp-auth` function. The two are
        // mutually exclusive: r_liteprofile is deprecated and ungrantable for
        // apps created after Aug 2023, and requesting it alongside `openid`
        // returns unauthorized_scope_error. That is why the product has its own
        // authorize call rather than a flag on this one.
      ...
      // The standalone MCP product's auth actions (linkedin_signin,
      // connect_linkedin, link_mcp_key, revoke_mcp_key) live in that product's
      // own repo — geryslov/ads-manager-hub-2bd81d31, function `mcp-auth`.
      // They are deliberately NOT here: this function is the shared LinkedIn
      // reporting surface, and the product should not need a fork of it.
