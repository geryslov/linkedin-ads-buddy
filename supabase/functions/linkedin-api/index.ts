  ...
  try {
    ...
    switch (action) {
      case 'get_auth_url': {
        ...
        //
        // 'legacy' (default) is what the linkedin-ads-buddy dashboard has always
        // requested. It keeps r_liteprofile because `get_profile` calls
        // /v2/me, which requires it — dropping it here would break the live app.
        //
        // 'oidc' is for the standalone MCP site. r_liteprofile is deprecated and
        // ungrantable for apps created after Aug 2023, and mixing it with the
        // OIDC scopes in one request produces unauthorized_scope_error — so the
        // two sets are mutually exclusive, not additive. w_member_social is
        // dropped too: no MCP tool posts, so it is pure consent friction.
        // OIDC callers should read identity from /v2/userinfo, not /v2/me.
        //
        // Flip the default to 'oidc' once the old dashboard is retired.
        const scopeSet = params?.scopeSet === 'oidc' ? 'oidc' : 'legacy';
        const scope = scopeSet === 'oidc'
      ...
      // ── Standalone MCP product: JWT-scoped key + connection management ──────
      // These replace sync_mcp_token, which is service-role with zero caller
      // auth (anyone can overwrite any key's token). Delete sync_mcp_token once
      // the old dashboard is off it.

      case 'link_mcp_key': {
        // Mint or rotate the caller's MCP API key. The key is scoped to the
        // Supabase user derived from the JWT — never from the request body.
        ...
        const { rotate, providerToken, expiresIn, label } = params || {};

        const { data: existing } = await supabaseClient
          .from('mcp_keys')
          .select('api_key, linkedin_token, linkedin_token_expires_at')
          .eq('user_id', userId)
          .eq('status', 'active')
          .maybeSingle();

        // Never overwrite a stored token with null. provider_token is absent on
        // TOKEN_REFRESHED and on SIGNED_IN events restored from storage, and
        // the client calls this on every mount.
        const linkedinToken = providerToken || existing?.linkedin_token;
        if (!linkedinToken) {
          return new Response(JSON.stringify({
            error: 'no_linkedin_token',
            message: 'No LinkedIn token on file. Complete the LinkedIn connection first.',
          }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
        }

        const expiresAt = providerToken && expiresIn
          ? new Date(Date.now() + Number(expiresIn) * 1000).toISOString()
          : existing?.linkedin_token_expires_at ?? null;

        if (existing && rotate) {
          await supabaseClient.from('mcp_keys')
            .update({ status: 'revoked', revoked_at: new Date().toISOString() })
            .eq('api_key', existing.api_key);
        }

        const apiKey = (existing && !rotate) ? existing.api_key : crypto.randomUUID();

        const { error: upsertErr } = await supabaseClient.from('mcp_keys').upsert({
          api_key: apiKey,
          user_id: userId,
        ...
        if (upsertErr) {
          console.error('link_mcp_key failed:', upsertErr);
          return new Response(JSON.stringify({ error: upsertErr.message }), {
            status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          });
        }

        return new Response(JSON.stringify({ ok: true, apiKey, expiresAt }), {
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
      }

      case 'connect_linkedin': {
        // Server-side LinkedIn code exchange. Unlike exchange_token, the access
        // token is stored and never returned to the browser. This is also the
        // day-60 reconnect path.
        const authHeader = req.headers.get('Authorization') || '';
        const jwt = authHeader.replace(/^Bearer\s+/i, '');
        const { data: userData, error: authErr } = await supabaseClient.auth.getUser(jwt);
        if (authErr || !userData?.user) {
        ...
        if (!tokenResponse.ok || !tokenData.access_token) {
          console.error('connect_linkedin exchange failed:', tokenResponse.status);
          return new Response(JSON.stringify({
            error: 'exchange_failed',
            message: tokenData.error_description || 'LinkedIn rejected the authorization code.',
          }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
        }
        ...
        if (upsertErr) {
          console.error('connect_linkedin store failed:', upsertErr);
          return new Response(JSON.stringify({ error: upsertErr.message }), {
            status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          });
        }

        // Note whether LinkedIn issued a refresh token — that single fact
        // determines whether reconnects can ever be made silent.
        return new Response(JSON.stringify({
          ok: true,
          apiKey,
          expiresAt,
      ...
      case 'linkedin_signin': {
        // LinkedIn sign-in WITHOUT Supabase's linkedin_oidc provider.
        //
        // Why this exists: enabling that provider is a dashboard step, and the
        // credentials it needs are already configured here as edge secrets. This
        // action does the whole thing server-side — exchange the code, read the
        // OIDC claims, find-or-create the Supabase user, store the ads token,
        // and hand back a one-time token the browser redeems for a session.
        //
        // Unauthenticated by necessity: the caller is signing IN. The security
        // boundary is the LinkedIn authorization code itself — single-use,
        // short-lived, and only redeemable with our client_id + client_secret.
        ...
        const tokenData = await tokenResponse.json();
        if (!tokenResponse.ok || !tokenData.access_token) {
          console.error('linkedin_signin exchange failed:', tokenResponse.status, tokenData?.error);
          return new Response(JSON.stringify({
            error: 'exchange_failed',
            message: tokenData.error_description || 'LinkedIn rejected the authorization code.',
          }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
        ...
        // /v2/userinfo is the OIDC identity endpoint. /v2/me is the old one and
        // needs the deprecated r_liteprofile scope, which cannot be combined
        // with openid — so identity must come from here.
        const userinfoRes = await fetch('https://api.linkedin.com/v2/userinfo', {
          headers: { 'Authorization': `Bearer ${tokenData.access_token}` },
        });
        const claims = await userinfoRes.json();
        if (!userinfoRes.ok || !claims?.email) {
          console.error('linkedin_signin userinfo failed:', userinfoRes.status, claims);
          return new Response(JSON.stringify({
            error: 'userinfo_failed',
            message: claims?.message
              || 'Could not read your LinkedIn profile. The app may be missing the "Sign In with LinkedIn using OpenID Connect" product.',
          }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
        }
        ...
        // Create if new; ignore "already registered". handle_new_user() fills in
        // profiles + the default role from this metadata.
        const { error: createErr } = await supabaseClient.auth.admin.createUser({
          email, email_confirm: true, user_metadata: meta,
        });
        if (createErr && !/already|exists|registered/i.test(createErr.message)) {
          console.error('linkedin_signin createUser failed:', createErr);
          return new Response(JSON.stringify({ error: createErr.message }), {
            status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          });
        }

        // generateLink returns both the user (so we get its id) and a one-time
        // hashed_token the browser redeems via verifyOtp. No email is sent.
        ...
        if (linkErr || !linkData?.properties?.hashed_token || !linkData?.user?.id) {
          console.error('linkedin_signin generateLink failed:', linkErr);
          return new Response(JSON.stringify({
            error: 'session_failed',
            message: linkErr?.message || 'Could not establish a session.',
          }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
        }
        ...
        const { data: existingKey } = await supabaseClient
          .from('mcp_keys').select('api_key').eq('user_id', userId).eq('status', 'active').maybeSingle();
        const apiKey = existingKey?.api_key ?? crypto.randomUUID();

        const { error: keyErr } = await supabaseClient.from('mcp_keys').upsert({
          api_key: apiKey,
          user_id: userId,
          linkedin_token: tokenData.access_token,
          linkedin_token_expires_at: expiresAt,
        ...
        if (keyErr) console.error('linkedin_signin key upsert failed:', keyErr);
      ...
      case 'revoke_mcp_key': {
        ...
        // Scoped to the caller's own keys — a user cannot revoke someone else's.
        const { error } = await supabaseClient.from('mcp_keys')
          .update({ status: 'revoked', revoked_at: new Date().toISOString() })
      ...
      case 'update_campaign_targeting': {
        ...
        let { campaignId, campaignIds, titleUrns, skillUrns, mode } = params;
