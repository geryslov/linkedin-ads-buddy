...
- **Widget kit** — [widgets.tsx](src/components/dashboard/widgets.tsx): `WidgetCard` (card shell with title/subtitle/toolbar header), `EmptyState`, `StatusPill` (semantic tones: success/warning/danger/info/neutral), `SegmentedControl`, `ChartLegend`. Most dashboard panels build from these. (Not yet converted: MegaBudgetPacingDashboard, BudgetPacingDashboard, LeadGenAnalyzer, CompanyInfluenceMatcher — they still inherit tokens but keep their old internal layout.)
