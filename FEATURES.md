...
**Audience Templates** — reusable named audiences (include + exclude lists of titles/skills/companies/industries) stored in `saved_targeting_audiences` (new `exclude_entities` column). Assign an audience to any set of campaigns; assignments persist in `audience_campaign_assignments` (audience_id × campaign_id, last sync time/status) so an edited audience can be re-synced to all its campaigns. Applying calls `update_campaign_targeting` in `replace` mode with the include set, then `exclude` mode with the exclusion set. Platform-only write (canWrite gated).
