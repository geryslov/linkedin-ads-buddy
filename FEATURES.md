...
| Budget Pacing | [BudgetPacingDashboard](src/components/dashboard/BudgetPacingDashboard.tsx), [MegaBudgetPacingDashboard](src/components/dashboard/MegaBudgetPacingDashboard.tsx) — per-account LinkedIn, Google, and Additional pacing lanes with inline budget/spend editing; each client can be linked to a Google Ads customer ([GoogleAccountLinkPicker](src/components/dashboard/GoogleAccountLinkPicker.tsx)) so Google spend is pulled live instead of typed | [useBudgetPacing](src/hooks/useBudgetPacing.ts), [useMegaBudgetPacing](src/hooks/useMegaBudgetPacing.ts), [useGoogleAdsLinks](src/hooks/useGoogleAdsLinks.ts) |
...
**Campaign Editor** — bulk targeting edits across multiple campaigns. Job titles, skills, companies, and industries can be added, replaced, or excluded. Add mode merges companies/industries into the campaign's existing organization OR clause, including uploaded company-list audience facets, so one-off additions widen that organization targeting instead of creating a new AND constraint. Backend `update_campaign_targeting`. Moved here from Reports (it is a bulk operation). Campaign writes use the connected LinkedIn access token and verify the user's account role directly with LinkedIn, so reconnecting LinkedIn is sufficient and no separate app sign-in is required.
