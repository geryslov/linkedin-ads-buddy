...
**Campaign Editor** — bulk targeting edits across multiple campaigns. Job titles, skills, companies, and industries can be added, replaced, or excluded; company/industry selections default to Exclude so they are merged into the exclusion OR layer without narrowing included targeting. Backend `update_campaign_targeting`. Moved here from Reports (it is a bulk operation).
